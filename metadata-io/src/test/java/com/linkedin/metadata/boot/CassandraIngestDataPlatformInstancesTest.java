...
public class CassandraIngestDataPlatformInstancesTest {
  ...
  public void testIt() throws Exception {
    ...
    List<Urn> corpUserUrns = ingestThisManyCorpUserInfoAspects(1234, metadata);
    ...
    List<Urn> chartUrns = ingestThisManyChartInfoAspects(1234, metadata);
    ...
    List<EntityAspect> chartAspects = chartUrns.stream()
        .map(urn -> _aspectDao.getLatestAspect(urn.toString(), DATA_PLATFORM_INSTANCE_ASPECT_NAME))
        .collect(Collectors.toList());
    assertEquals(chartUrns.size(), chartAspects.size(), "We should get as many aspects as we have URNs.");
    int i = 0;
    for (EntityAspect aspect : chartAspects) {
      if (aspect != null) {
        i++;
      }
    }
//    assertEquals(chartUrns.size(), i);
